...
const UserLogin = async (email, password) => {
	...
	try {
		const response = await axios.get(
			`${apiURL}/api/user/${email}?password=${password}`,
		);
		return { status: response.data.status, data: response.data.user };
	} catch (error) {
...
/**
 * Updates the location of a delivery associate
 * @param {string} userId - The delivery associate ID
 * @param {Object} location - Location coordinates
 * @param {number} location.latitude - Latitude
 * @param {number} location.longitude - Longitude
 * @returns {Promise<Object>} Response status and data
 */
const updateDeliveryLocation = async (userId, location) => {
	try {
		const response = await axios.post(`${apiURL}/api/mapping`, {
			daid: userId,
			latitude: location.latitude,
			longitude: location.longitude,
			date: new Date().toISOString(),
			// These would need to be provided in a real implementation
			requestid: "65f1234567890abcdef12345", // Placeholder
			supplierid: "65f1234567890abcdef12346", // Placeholder
		});
		return { status: response.status, data: response.data };
	} catch (error) {
...
/**
 * Gets the last known location of a delivery associate
 * @param {string} userId - The delivery associate ID
 * @returns {Promise<Object>} Location data
 */
const getDeliveryLocation = async (userId) => {
	try {
		const response = await axios.get(`${apiURL}/api/mapping/${userId}`);
...
/**
 * Gets all locations for a delivery associate
 * @param {string} userId - The delivery associate ID
 * @returns {Promise<Array>} Array of location records
 */
const getAllDeliveryLocations = async (userId) => {
	try {
		const response = await axios.get(`${apiURL}/api/mapping`, {
			params: {
				daid: userId,
			},
		});
